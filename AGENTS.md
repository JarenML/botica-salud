...
- Gotcha (hit 4 times already, across `index.css`, `auth.css`, `client.css`, `register_sale.css`): **any legacy CSS rule not inside a Tailwind `@layer` always wins over Tailwind's utilities, regardless of specificity** — this includes global resets (`* { padding: 0 }`, breaks every `p-*`/`m-*`), plain-sounding class names that collide with real Tailwind utilities (a legacy `.hidden { display: none }` silently defeats `lg:flex`), and bare tag selectors (a legacy `h2 { color: #4cc9f0 }` silently recolors every `<h2>` in the app, including ones on screens that already migrated). Since App.jsx statically imports every page, every legacy `.css` file's rules are present on every route, not just its own. Before migrating a screen, grep the other legacy `.css` files for bare rules using Tailwind-shaped class names *or* bare tag selectors, and either wrap them in `@layer base`/`@layer components`, or (preferred for tag selectors) scope them to the component's actual class so they can't leak onto unrelated elements even within the layer.
- **z-index stacking:** the sticky `Header` (`header.css`) sits at `z-index: 1000`, higher than any Tailwind default (`z-50` = 50). Any modal/dialog/overlay must use `z-[1100]` or higher, or it renders *behind* the header instead of covering it — this already happened once in Inventario's product modal. The Header's own dropdown menu is `z-index: 100`.
